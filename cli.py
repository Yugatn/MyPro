...
def import_project(path: Path, project_dir: Path, dry_run: bool = True):
    ...
    if not dry_run:
        p.log.append(type="proposal.import_project",
                     actor=f"adapter:{adapter.adapter_id}@{adapter.adapter_version}",
                     payload=payload)
...
def bundle_export(project_dir: Path, output: Path,
                  include_media: bool = typer.Option(False, help="Reserved for media-aware bundles.")):
    from core.bundle import export_bundle
    digest = export_bundle(project_dir, output, include_media=include_media)
    typer.echo(f"bundle: {digest}")

@app.command("bundle-verify")
def bundle_verify(bundle: Path):
    from core.bundle import verify_bundle
    result = verify_bundle(bundle)
    typer.echo(f"ok={result.ok} events={result.event_count} tip={result.tip_hash}")
    for problem in result.problems:
        typer.echo(f"  - {problem}", err=True)
    raise typer.Exit(code=0 if result.ok else 1)

@app.command("media-unresolved")
def media_unresolved(project_dir: Path):
    """Check explicitly registered media when a session-level locator is supplied."""
    typer.echo("media locator is available as a library API; no canonical filesystem paths are inferred.")
